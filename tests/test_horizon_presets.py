...
def _make_universe(rows: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    numeric_cols = [
        "MarketCap", "AvgVolume", "YearsListed", "Altman_Z", "data_completeness",
        "Composite_Score", "Valuation_Score", "Health_Score", "Profitability_Score",
        "Growth_Score", "Shareholder_Score", "Risk_Score_v2", "EarningsQuality_Score",
        "Moat_Score", "Momentum_Score", "ROIC", "ROE", "PB", "PE", "PFCF",
        "DivYield", "PayoutRatio", "InterestCoverage", "FCF", "FCFMargin",
        "OperatingMargin", "RevenueGrowth", "Beta", "EV_EBITDA", "EBITDA",
        "EBIT", "TotalDebt", "Cash", "Piotroski_F", "DCF_MoS_Mid",
        "Volume_Surge", "Momentum_12_1", "RS_3m", "EPS_Rev_90d", "EPS_Rev_180d",
        "SUE", "Sentiment_Score", "score_lt", "score_mt", "score_st",
    ]
...
class TestRegistry:
    def test_all_9_presets_present(self):
        expected = {
            "LT_QUALITY_COMPOUNDER", "LT_PEA_DEFENSIVE", "LT_DEEP_VALUE",
            "MT_GARP", "MT_TURNAROUND", "MT_INCOME",
            "ST_MOMENTUM_QUALITY", "ST_EARNINGS_DRIFT", "ST_OVERSOLD_BOUNCE",
        }
    ...
    def test_st_presets_carry_cto_flag(self):
        for name in ("ST_MOMENTUM_QUALITY", "ST_EARNINGS_DRIFT", "ST_OVERSOLD_BOUNCE"):
            ...
            assert p.get("recommended_account") == "CTO", (
                f"{name} must carry recommended_account='CTO'"
            )
            assert p.get("pea_warning") is True, (
                f"{name} must carry pea_warning=True"
            )
    ...
    def test_every_preset_has_universal_prefilters(self):
        ...
        for name, p in PRESET_REGISTRY.items():
            for key in required:
                assert key in p["filters"], (
                    f"Preset {name} is missing universal filter '{key}'"
                )
...
class TestLTQualityCompounder:
    ...
    def test_no_crash_missing_optional_columns(self):
        ...
        cols_to_drop = [
            "ROIC_5y_Avg", "CashConversionRatio_3y", "Gross_Profitability",
            "RevenueCAGR_5y", "EPS_CAGR_5y", "PosRevenueYears_5y",
...
class TestLTPEADefensive:
    ...
    def test_pea_only_enforced(self):
        ...
        rows = [
            _row(Ticker="EU_CO", PEA=True, data_completeness=0.85,
                 Piotroski_F=8, Altman_Z=3.0, MarketCap=2_000_000_000,
                 Beta=0.80, DivYield=0.025),
            _row(Ticker="US_CO", PEA=False, data_completeness=0.85,
                 Piotroski_F=8, Altman_Z=3.0, MarketCap=2_000_000_000,
                 Beta=0.80, DivYield=0.025),
        ]
    ...
    def test_high_beta_excluded(self):
        ...
        df = _build_30_rows("HIGHVOL", {
            "Beta": 1.3, "PEA": True, "data_completeness": 0.85,
            "Piotroski_F": 8, "Altman_Z": 3.0, "MarketCap": 2_000_000_000,
            "DivYield": 0.025,
        })
        result = apply_filters(df, self.PRESET["filters"], pea_only=True)
    ...
    def test_no_dividend_excluded(self):
        ...
        df = _build_30_rows("NODIV", {
            "DivYield": 0.005, "PEA": True, "data_completeness": 0.85,
            "Piotroski_F": 8, "Altman_Z": 3.0, "MarketCap": 2_000_000_000,
            "Beta": 0.80,
        })
        result = apply_filters(df, self.PRESET["filters"], pea_only=True)
    ...
    def test_market_cap_floor_1b(self):
        ...
        df = _build_30_rows("SMALL_EU", {
            "PEA": True, "MarketCap": 400_000_000, "data_completeness": 0.85,
            "Piotroski_F": 8, "Altman_Z": 3.0, "Beta": 0.80, "DivYield": 0.025,
        })
        result = apply_filters(df, self.PRESET["filters"], pea_only=True)
        assert "SMALL_EU" not in result.index

    def test_perfect_pea_defensive_passes(self):
        df = _build_30_rows("PERFECT_PEA", {
            "PEA": True, "Beta": 0.70, "DivYield": 0.03, "PayoutRatio": 0.50,
            "MarketCap": 2_000_000_000, "data_completeness": 0.85,
            "Piotroski_F": 8, "Altman_Z": 3.2, "InterestCoverage": 9.0,
            "FCFMargin": 0.12, "ROIC": 0.15,
        })
        result = apply_filters(df, self.PRESET["filters"], pea_only=True)
...
class TestLTDeepValue:
    ...
    def test_deep_value_edge_case_f_score_6(self):
        ...
        _dv_extras = {
            "Altman_Z": 2.0, "Valuation_Score": 20.0,
            "PB": 1.2, "DCF_MoS_Mid": 0.35,
    ...
    def test_altman_z_floor_1_8(self):
        ...
        _dv_extras = {"Piotroski_F": 6, "Valuation_Score": 20.0,
                      "PB": 1.2, "DCF_MoS_Mid": 0.35, "EV_EBITDA": 6.0}
    ...
    def test_expensive_on_pb_excluded(self):
        ...
        df = _build_30_rows("EXP_PB", {
            "PB": 2.5, "Piotroski_F": 6, "Altman_Z": 2.0,
            "Valuation_Score": 20.0, "DCF_MoS_Mid": 0.35,
        })
        result = apply_filters(df, self.PRESET["filters"])
    ...
    def test_insufficient_dcf_mos_excluded(self):
        ...
        df = _build_30_rows("LOWMOS", {
            "DCF_MoS_Mid": 0.10, "Piotroski_F": 6, "Altman_Z": 2.0,
            "PB": 1.2, "Valuation_Score": 20.0,
        })
        result = apply_filters(df, self.PRESET["filters"])
...
class TestMTTurnaround:
    ...
    def test_negative_fcf_excluded(self):
        ...
        df = _build_30_rows("NEG_FCF_TURN", {
    ...
    def test_negative_cash_excluded(self):
        ...
        df = _build_30_rows("NO_CASH_TURN", {
            "Cash": -5_000_000,
            "FCF": 30_000_000,
            "Altman_Z": 1.6,
        })
    ...
    def test_distressed_excluded(self):
        ...
        df = _build_30_rows("BANKRUPT_TURN", {
            "Altman_Z": 1.0,
            "FCF": 30_000_000,
            "Cash": 100_000_000,
        })
...
class TestMTIncome:
    ...
    def test_nan_optional_columns_pass(self):
        ...
        df = _build_30_rows("DIV_NAN_COLS", {
            "DivYield": 0.04, "PayoutRatio": 0.55,
            "Piotroski_F": 7, "Altman_Z": 2.5,
        })
...
class TestSTMomentumQuality:
    ...
    def test_below_50dma_excluded(self):
        ...
        df = _build_30_rows("BELOW50", {
            "Above_50DMA": False, "Above_200DMA": True,
            "MarketCap": 1_000_000_000,
        })
        result = apply_filters(df, self.PRESET["filters"])
    ...
    def test_no_golden_cross_excluded(self):
        ...
        df = _build_30_rows("NO_GX", {
            "Golden_Cross": False, "Above_50DMA": True, "Above_200DMA": True,
            "MarketCap": 1_000_000_000,
        })
        result = apply_filters(df, self.PRESET["filters"])
    ...
    def test_high_beta_excluded(self):
        ...
        df = _build_30_rows("HYPERVOL", {
            "Beta": 2.1, "MarketCap": 1_000_000_000,
        })
        result = apply_filters(df, self.PRESET["filters"])
...
class TestSTOversoldBounce:
    ...
    def test_distressed_excluded(self):
        ...
        df = _build_30_rows("DISTRESS_OB", {
            "Altman_Z": 0.8, "Above_200DMA": True,
            "MarketCap": 1_000_000_000,
        })
        result = apply_filters(df, self.PRESET["filters"])
    ...
    def test_rsi2_gap_documented(self):
        ...
        df = _build_30_rows("OVERSOLD_NO_RSI", {
            "Above_200DMA": True, "Altman_Z": 2.0,
            "MarketCap": 1_000_000_000,
        })
...
class TestScreenHorizonPreset:
    ...
    def test_returns_dataframe(self):
        df = _build_30_rows("COMPOUNDER", {
            "Piotroski_F": 8, "Altman_Z": 3.0, "data_completeness": 0.85,
        })
    ...
    def test_sorted_by_score_lt(self):
        ...
        rows = [
            _row(Ticker="A_HIGH", score_lt=80.0, passes_gates_lt=True,
                 Piotroski_F=8, Altman_Z=3.0, data_completeness=0.85),
            _row(Ticker="B_MED", score_lt=65.0, passes_gates_lt=True,
                 Piotroski_F=8, Altman_Z=3.0, data_completeness=0.85),
            _row(Ticker="C_LOW", score_lt=50.0, passes_gates_lt=True,
                 Piotroski_F=8, Altman_Z=3.0, data_completeness=0.85),
        ]
        df = _make_universe(rows)
        result = screen_horizon_preset(df, "LT_QUALITY_COMPOUNDER")
    ...
    def test_passes_gates_filter_applied(self):
        ...
        rows = [
            _row(Ticker="GATE_PASS", passes_gates_lt=True, score_lt=75.0,
                 Piotroski_F=8, Altman_Z=3.0, data_completeness=0.85),
            _row(Ticker="GATE_FAIL", passes_gates_lt=False, score_lt=75.0,
                 Piotroski_F=8, Altman_Z=3.0, data_completeness=0.85),
        ]
        df = _make_universe(rows)
        result = screen_horizon_preset(df, "LT_QUALITY_COMPOUNDER")
    ...
    def test_st_preset_sorted_by_score_st(self):
        rows = [
            _row(Ticker="ST_A", score_st=70.0, passes_gates_st=True,
                 MarketCap=1_000_000_000, Above_50DMA=True, Above_200DMA=True,
                 Golden_Cross=True, Volume_Surge=1.5, SUE=1.2),
            _row(Ticker="ST_B", score_st=55.0, passes_gates_st=True,
                 MarketCap=1_000_000_000, Above_50DMA=True, Above_200DMA=True,
                 Golden_Cross=True, Volume_Surge=1.5, SUE=1.2),
        ]
        df = _make_universe(rows)
    ...
    def test_mt_preset_sorted_by_score_mt(self):
        rows = [
            _row(Ticker="MT_HIGH", score_mt=72.0, passes_gates_mt=True,
                 DivYield=0.04, PayoutRatio=0.55, Piotroski_F=7),
            _row(Ticker="MT_LOW", score_mt=56.0, passes_gates_mt=True,
                 DivYield=0.04, PayoutRatio=0.55, Piotroski_F=7),
        ]
        df = _make_universe(rows)
        result = screen_horizon_preset(df, "MT_INCOME")
...
class TestApplyFiltersBackwardCompat:
    ...
    def test_quality_growth_filters_still_work(self):
        ...
        rows = [
            _row(Ticker="QUALITY", ROE=0.20, OperatingMargin=0.15,
                 RevenueGrowth=0.10, Composite_Score=70.0,
                 MarketCap=600_000_000),
            _row(Ticker="LOWROE", ROE=0.05, OperatingMargin=0.15,
                 RevenueGrowth=0.10, Composite_Score=70.0,
                 MarketCap=600_000_000),
        ]
        df = _make_universe(rows)
    ...
    def test_dividend_filters_still_work(self):
        ...
        rows = [
            _row(Ticker="DIV", DivYield=0.04, PayoutRatio=0.60,
                 InterestCoverage=5.0, MarketCap=2_000_000_000,
                 Composite_Score=55.0),
            _row(Ticker="NO_DIV", DivYield=0.01, PayoutRatio=0.60,
                 InterestCoverage=5.0, MarketCap=2_000_000_000,
                 Composite_Score=55.0),
        ]
        df = _make_universe(rows)
...
class TestUniversalPreFilters:
    ...
    def test_nan_passes_universal_filters(self):
        ...
        rows = [_row(Ticker="NAN_ROW", Altman_Z=float("nan"),
                     data_completeness=float("nan"), YearsListed=float("nan"))]
        df = _make_universe(rows)
        result = apply_filters(df, self._minimal_filters())
