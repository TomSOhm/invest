...
class TestProtocolConformance:
    ...
    def test_implements_protocol(self, yf: YFinanceDataFetcher) -> None:
        assert isinstance(yf, MarketDataSource), (
            "YFinanceDataFetcher does not satisfy MarketDataSource Protocol"
        )
    ...
    def test_has_all_required_methods(self, yf: YFinanceDataFetcher) -> None:
        ...
        for method in required_methods:
            assert hasattr(yf, method) and callable(getattr(yf, method)), (
                f"Missing required method: {method}"
            )

    def test_fetch_eps_revisions_raises_not_implemented(
...
class TestLiveYFinanceFetch:
    ...
    def test_fetch_fundamentals_annual_returns_dataframe(
        self, yf: YFinanceDataFetcher
    ) -> None:
        import pandas as pd
    ...
    def test_fetch_full_row_covers_scoring_columns(
        self, yf: YFinanceDataFetcher
    ) -> None:
        import math
    ...
    def test_fetch_full_row_pea_false_for_us_ticker(
        self, yf: YFinanceDataFetcher
    ) -> None:
        row = yf.fetch_full_row("AAPL")
    ...
    def test_fetch_full_row_pea_true_for_french_ticker(
        self, yf: YFinanceDataFetcher
    ) -> None:
