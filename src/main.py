...
def get_pme_tickers_from_source(df: pd.DataFrame, source_type="example_list"):
    ...
    if source_type == "example_list":
        ...
        tickers = ["ALCAR.PA", "ALVDM.PA", "ALNEV.PA", "MCPHY.PA", "ABIO.PA"]
        # "ALTBG.PA",
    ...
    try:
        ...
        if pd.isna(pb_ratio):
            try:
                ...
                if (
                    not balance.empty
                    and "Total Stockholder Equity" in balance.index
                    and pd.notna(shares)
                    and shares != 0
                ):
                    equity = balance.loc["Total Stockholder Equity"].iloc[0]
        ...
        return {
            ...
            "ROE": np.nan,
            "P/B": np.nan,
            "Croissance CA (%)": np.nan,
            "VE (M)": np.nan,
            "VE/CA": np.nan,
            "VE/EBE": np.nan,
            "Dette Nette/EBE": np.nan,
            "Gearing": np.nan,
            "Vol. Moyen Val. (k)": np.nan,
        }
...
if __name__ == "__main__":
    ...
    if USE_MULTIPROCESSING:
        with multiprocessing.Pool(processes=min(3, multiprocessing.cpu_count())) as pool:
            for data in tqdm(
                pool.imap(get_company_data, LISTE_TICKERS_PME),
                total=len(LISTE_TICKERS_PME),
                desc="Extraction des données",
            ):
    ...
    if not df_plot.empty:
        ...
        criteria = {
            ...
            "Croissance CA (%)": {
                "min": SEUIL_CROISSANCE_CA_MIN * 100,
                "max": 50,
                "inverse": False,
                "allowNegativeAsGood": False,
            },
            "Dette Nette/EBE": {
                "min": 0,
                "max": SEUIL_DETTE_NETTE_EBE_MAX,
                "inverse": True,
                "allowNegativeAsGood": True,
            },
            "Gearing": {"min": 0, "max": SEUIL_GEARING_MAX, "inverse": True, "allowNegativeAsGood": True},
            "Vol. Moyen Val. (k)": {
                "min": SEUIL_VOLUME_MOYEN_MIN_VALEUR / 1000,
                "max": 1000,
                "inverse": False,
                "allowNegativeAsGood": False,
            },
            ...
            for col in fig_table_all_cols_exist:
                if col in criteria:
                    ...
                    score, color = score_and_color(
                        row[col], col, crit["min"], crit["max"], crit["inverse"], crit.get("allowNegativeAsGood", False)
                    )
                    print(
                        f"{row.name} | {col} = {row[col]} | score = {score} | color = {color} | {'GOOD' if score > 0.5 else 'BAD'}"
                    )
                    scores.append(score)
        ...
        for _i, (row, score) in enumerate(zip(cell_colors, score_global, strict=False)):
            row.append(to_hex(GLOBAL_CMAP(score if not np.isnan(score) else 0.5)))

        # Ajoute la couleur pour la colonne score global
        cell_colors = [
            row + [to_hex(GLOBAL_CMAP(score if not np.isnan(score) else 0.5))]
            for row, score in zip(cell_colors, score_global, strict=False)
        ]
            ...
            data=[
                go.Table(
                    ...
                    cells=dict(
                        ...
                        fill_color=[["#ffffff"] * len(df_for_table)] + list(map(list, zip(*cell_colors, strict=False))),
                        align="left",
        ...
        fig_table_all.update_layout(
            title_text="Tableau de Bord des Entreprises Analysées (Coloration par critère)",
            height=max(400, 50 + len(df_plot) * 35),
        )
        fig_table_all.show()
        ...
        if not df_plot_pe_roe.empty:
            ...
            fig_pe_roe.add_hline(
                ...
                line_dash="dash",
                line_color="blue",
                annotation_text=f"ROE > {SEUIL_ROE_MIN * 100:.0f}%",
            )
        ...
        if "Croissance CA (%)" in df_plot.columns:
            df_plot_croissance = df_plot.dropna(subset=["Croissance CA (%)"]).sort_values(
            ...
            if not df_plot_croissance.empty:
                ...
                fig_croissance.add_hline(
                    y=SEUIL_CROISSANCE_CA_MIN * 100,
                    line_dash="dash",
                    line_color="orange",
                    annotation_text=f"Croissance > {SEUIL_CROISSANCE_CA_MIN * 100:.0f}%",
                )
        ...
        if "Dette Nette/EBE" in df_plot.columns and "Gearing" in df_plot.columns:
            ...
            if not df_plot_debt.empty:
                ...
                fig_debt.add_hline(
                    ...
                    line_dash="dash",
                    line_color="orange",
                    annotation_text=f"Gearing < {SEUIL_GEARING_MAX}",
                )
