...
def chart_composite_scores(scored_df, output_path):
    ...
    for i, (bar, score) in enumerate(zip(bars, top20["Composite_Score"], strict=False)):
...
def chart_pe_vs_roe(scored_df, output_path):
    ...
    for ticker, row in df.iterrows():
        ax.annotate(
            ticker.replace(".PA", "").replace(".DE", "").replace(".MI", "").replace(".AS", "").replace(".MC", ""),
            (row["PE"], row["ROE"] * 100),
            fontsize=7,
            color="#ccc",
...
def chart_score_radar(scored_df, output_path, n=5):
    ...
    for i, (ticker, row) in enumerate(top_n.iterrows()):
        ...
        ax.plot(
            ...
            "o-",
            linewidth=2,
            label=f"{name} ({row.get('Composite_Score', 0):.0f})",
