...
class ChartMetrics(BaseModel):
    ...
    beta: float | None = Field(None, description="cov(stock, benchmark) / var(benchmark); None when no benchmark or < 60 obs")
...
class MovingAverages(BaseModel):
    ...
    ma_200: list[float | None] = Field(default_factory=list, description="200d rolling mean of close, aligned to candles")
